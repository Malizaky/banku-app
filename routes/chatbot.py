...
from models import ChatbotFlow, ChatbotQuestion, ChatbotResponse, ChatbotStepBlock, ItemType, DataStorageMapping, ChatbotCompletion, Item, Profile, Bank, db
...
@chatbot_bp.route('/')
@login_required
@require_permission('chatbots', 'read')
def index():
...
def start_flow(flow_id):
    ...
    if organization_id:
        session['organization_id'] = int(organization_id)
...
def handle_file_upload(flow_id):
    ...
    try:
        ...
        upload_dir = os.path.join(current_app.instance_path, 'uploads', 'chatbot', str(flow_id), str(question_id), file_type)
        ...
        return jsonify({
            'success': True,
                'file_info': {
                    ...
                    'mime_type': mime_type,
                    'path': f"uploads/chatbot/{flow_id}/{question_id}/{file_type}/{unique_filename}"
                }
        })
...
def complete_flow_with_storage_logic(flow_id, collected_data):
    ...
    try:
        ...
        # Find associated item type and data mapping
        item_type = ItemType.query.filter_by(chatbot_id=flow_id, is_active=True).first()
        
        if item_type:
            completion.item_type_id = item_type.id
            ...
            elif item_type.bank_id:
                # Fallback to item type bank assignment
                bank_id = item_type.bank_id
                data_mapping = {}  # Use default mapping
            ...
            completion.storage_status = 'failed'
            completion.error_message = 'No item type associated with this chatbot'
...
def create_item_from_chatbot_data(processed_data, item_type, bank_id):
    ...
    try:
        ...
        # Handle category - use processed data if available, otherwise use item_type.name
        category = processed_data.get('category', item_type.name)
        ...
        unmapped_data = {}
        used_fields = set(['title', 'short_description', 'detailed_description', 'category', 'subcategory', 'tags', 'pricing_type', 'price', 'currency', 'location'])
        ...
        if unmapped_data:
            item.type_data = str(unmapped_data)  # Store as JSON string
