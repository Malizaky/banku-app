...
class Information(db.Model):
    ...
    # Relationships
    creator = db.relationship('User', foreign_keys=[created_by], backref='created_information')
    assignee = db.relationship('User', foreign_keys=[assigned_to], backref='assigned_information')
...
class ChatbotFlow(db.Model):
    ...
    questions = db.relationship('ChatbotQuestion', backref='flow', lazy=True, cascade='all, delete-orphan', order_by='ChatbotQuestion.order_index')
...
class ChatbotStepBlock(db.Model):
    ...
    # Relationships
    flow = db.relationship('ChatbotFlow', backref='step_blocks')
    creator = db.relationship('User', backref='created_step_blocks')
    questions = db.relationship('ChatbotQuestion', backref='step_block', lazy=True, cascade='all, delete-orphan', order_by='ChatbotQuestion.order_index')
...
class ButtonConfiguration(db.Model):
    ...
    # Relationships
    created_by = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    creator = db.relationship('User', backref='created_buttons')
...
class ItemType(db.Model):
    ...
    # Relationships
    created_by = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    creator = db.relationship('User', backref='created_item_types')
    chatbot = db.relationship('ChatbotFlow', backref='item_types')
    # Note: bank relationship can be accessed via queries when needed
...
class ChatbotCompletion(db.Model):
    ...
    # Relationships
    chatbot = db.relationship('ChatbotFlow', backref='completions')
...
class Organization(db.Model):
    ...
    owner = db.relationship('User', foreign_keys=[current_owner], backref='owned_organizations')
